...
def new_post():
    ...
    if forms.validate_on_submit():
        sanitized_content = getRenderend(forms.content.data)
        post=Userposts(user_id=current_user.id,title=forms.title.data,sub_heading=forms.sub_heading.data,content=sanitized_content,slug=forms.slug.data,background_img=forms.background_img.data,postby=current_user.username)
...
def update_post(sno):
    ...
    if form.validate_on_submit():
        ...
        post.content = getRenderend(form.content.data)
