...
from models import User, Userposts, db, getJson
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if check:
            ...
            secret_key = 'hardcoded_secret_1234567890_lab_key'
            token = user.get_reset_token(secret_key, expires_min=30)
            return render_template('store_token.html', params=params, token=token, next_page=next_page)
        else:
            flash('Login Unsuccessful. Please check username and password', 'danger')
    return render_template('userlogin.html', params=params, form=form)


@users.route('/api/post/<int:post_id>', methods=['GET'])
def api_post(post_id):
    ...
    secret_key = 'hardcoded_secret_1234567890_lab_key'
    payload = User.verify_reset_token(token, secret_key)
    if payload is None:
        return jsonify({'message': 'Invalid or expired token'}), 401

    post = Userposts.query.get_or_404(post_id)
    return jsonify({
        'id': post.sno,
        'title': post.title,
